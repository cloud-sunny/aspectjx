...
class JarMerger {
    ...
    JarMerger(File jarFile) throws IOException {
     ...
     */
    void setFilter(IZipEntryFilter filter) {
        this.filter = filter
    }

    void addFolder(File folder) throws IOException {
        init()
        try {
    ...
    private void addFolderWithPath(File folder, String path)
    ...
    void addJar(File file, boolean removeEntryTimestamp) throws IOException {
        init()
